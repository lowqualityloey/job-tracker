...
- **Current Milestone / Epic**: M0 **done** · M1 **MERGED** (PR #1) · M2a **MERGED** (PR #2 → `1ec8fe8`) · **M2b → PR #3 opened, awaiting human review (Level 2)** — <https://github.com/lowqualityloey/job-tracker/pull/3>
...
- **Current Working Branch**: `feat/m2b-filters-cross-tab` @ `bdec833` (== `origin/…`, **published**, 29 commits ahead of base). Base `main` @ `1ec8fe8`. **Phase advances via PR only** — see `AGENTS.md` §Branch & PR workflow
...
- **Verification Status**: at the branch tip — `npm run test:run` **100/100 in 13 files**, `tsc --noEmit` exit 0,
  ...
  **29-commit per-commit audit in a separate clone: 0 typecheck failures, the only 8 failing commits are
  exactly the 8 Red commits**. The audit was re-run after the last four commits landed, because the first
  version of that claim was made over 25 and stated as 29. Task-owned evidence: Task Record §6
