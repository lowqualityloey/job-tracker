...
- **CI Evidence**: **observed, not assumed** — [run 34551505046](https://github.com/lowqualityloey/job-tracker/actions/runs/34551505046),
  `pull_request`, 6/6 steps, **success in 43s**. The log shows the work happening rather than a step no-op'ing
  green: `npm ci` → *added 493 packages*, `Test Files 13 passed`, `✓ built in 1.12s`, then the guard step's own
  `✓ no config shadow, no leaked build output`
- **Current Branch / Revision**: `chore/debt-03-lint-and-ci`, published; base `main` @ `489cdb9` (merged PR #3)
