...
- **Last Updated**: 2026-09-10 22:25 UTC — M2a.4 landed (behaviours 012–017, 68 tests, DEBT-01/05/07 closed); AC-1…AC-10 met, AC-11 owed at M2a.5
...
1. **Agent**: M2a.5 — contract sweep across the seam (every `RepositoryError` code reachable and
   rendered), then the AC-11 whole-branch gate: `npm run build`, the full suite, and a manual reload check in a
   real browser (the residual DEBT-01 item this record has not yet discharged).
