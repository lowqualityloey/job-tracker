...
8. **Measurable targets** (dev machine, 5-50 rows — deliberately modest so a number is never unfalsifiable) — **all six, with their ratified thresholds intact and their status now measured:**
   ...
   **0** unhandled exceptions reaching a client (everything becomes a problem response) — **not a duration and not measurably countable post-hoc**: it is held by the exception-handling middleware plus `ProblemDetails`, and the suite's `500`-must-be-a-problem cases are the proxy for it. **A target with no command is a promise; §7 asks it to be deleted rather than defended, and it is here marked as defended-but-uninstrumented for the owner to strike or convert into an assertion**
   frontend bundle delta **< 2 kB**, which §2 assigned to **Slice 3 to "record the actual number from the build output"** — **NOT DONE, and not now doable from the repository**: no pre-M3 bundle size was ever recorded, so the delta's baseline does not exist. Current total gzipped JS is **60,136 bytes**, which is a measurement of nothing, because it is a level where a delta was asked for. **This is §2.8's one genuinely unmet instruction, and it was invisible to every one of my five earlier audits of this document because I audited it by re-reading, not by opening the line**
...
- [ ] **(`agent`-owned item, left open because it is genuinely NOT satisfied — not an owner approval.)** `grep -ci` over `docs/reviews/2026-09-11-m3-plan-grill.md`: `ENUM` **0 hits**, `timestamptz` **0**, `fan` **0**, `instance` **0** — so of this item's six required challenges the grill covered *`revision?: number`* (Q5), *the `xmin`-as-ETag counter* (Q6), *`If-Match` on `DELETE`* (Q3), *CORS* (Q4), *the §2 targets* (Q2) and *the eighth variant* (Q11), but **never challenged `text + CHECK` vs `ENUM`, client-minted ids, `date` vs `timestamptz`, or single-instance SSE fan-out.** **Four §7 boxes are the owner's; this one is mine and it is unfinished.**  (`pk:grill`) challenges at least: `text + CHECK` vs `ENUM`; client-minted ids; whether an opaque `revision?: number` on the domain type is "leaking transport into the domain" (my answer: it is a row version, and no page may read it — the field is the honest place to put it, and I expect that to be attacked); `date` vs `timestamptz`; single-instance SSE fan-out; whether keeping the local adapter is a bridge or a fork in the product
- [ ] The measurable targets in §2 are falsifiable by a command; if any cannot be, it is deleted rather than defended — **UN-CHECKED: my earlier check of this box rested on a false premise (see the gap 10b retraction). Re-read: four of six targets are falsifiable AND measured (GET, POST, SSE `< 500 ms`, API suite); the fifth (0 unhandled exceptions) is uninstrumented and needs either an assertion or deletion; the sixth (`< 2 kB` bundle delta) is **unmeetable from the repo** because no pre-M3 baseline was ever captured — §2 sent it to Slice 3 and Slice 3 did not record it. **The owner's call on all three.**
