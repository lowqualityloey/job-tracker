...
8. **Measurable targets** (dev machine, 5-50 rows — deliberately modest so a number is never unfalsifiable) — **all six, with their ratified thresholds intact and their status now measured:**
   `GET /api/applications` **p50 < 30 ms, p95 < 80 ms** — measured **p50 2.7 ms / p95 5.3 ms** (n=40, max 75.8) · **PASS**
   `POST` **p95 < 150 ms** including commit — measured **p50 4.7 / p95 5.8 ms** (n=25, max 52.9) · **PASS**
   **`SSE` notification observed in the second tab in < 500 ms** *(this is the ratified wording; see the retraction below)* — measured on **both** spans it can mean, and it passes on either: request→second-tab-visible **p50 33 / p95 88 / max 114 ms** (cold run) and **41 / 58 / 77 ms** (warm run), n=12+12, 24/24 observed · **PASS with ~5× headroom**
   API suite runs in **< 60 s** in CI — **29 s wall / 7 s xunit** measured locally on this tree (`time dotnet test`, 65 passed / 0 failed / exit 0) · **PASS, but this is a PROXY, not the CI figure**: the CI job adds PostgreSQL container start-up and a cold `dotnet restore`/build, and `runDuration` is `null` through the API on these runs, so **the ratified target's own instrument has not actually been read — the CI job's wall time still needs one** (`gh run view <id> --json jobs` or the Actions UI)
   **0** unhandled exceptions reaching a client (everything becomes a problem response) — **PASSES AS WRITTEN AND FAILS AS INTENDED: gap 12 falsified it.** A malformed `id` in a `POST` body returns **HTTP 500** (`JsonException` → `FormatException: The JSON value is not in a supported Guid format`), because `NewApplicationRequest` declares `Guid Id` — **so deserialization throws before `ApplicationValidation.Validate` ever runs**, and the request never reaches the pipeline that emits `400` + `errors[].pointer`. The exception handler does convert it to an RFC 9457 problem response, so **the literal target is met while the thing it existed to catch is broken**. A metric satisfiable in a way that hides its own purpose is worse than no metric: restate as **"no client-input error may surface as 5xx"** — which gap 12 fails, and which is a test rather than a promise
   frontend bundle delta **< 2 kB**, which §2 assigned to **Slice 3 to "record the actual number from the build output"** — **Slice 3 never did, and no baseline existed to compare against until now.** Measured 21:10 UTC from a real pre-M3 baseline (`git worktree` at `84bf560`; `package.json` and lockfile verified byte-identical, so the same `node_modules` applies): baseline **60,118 B** gzipped → M3-complete **61,359 B** = **+1,241 B (1.21 kB) · PASS**. **The configuration is part of the number**: measured with `VITE_API_BASE_URL` set, because with it unset Vite statically replaces the value, nothing references the adapter, **Rollup drops the module, and the flag-off bundle is byte-identical to the pre-M3 baseline at 60,118 B — a true 0 kB delta for a build that does not ship the adapter.** Reporting that zero would have been accurate, free, and meaningless: **a measurement of a build nobody ships is not a pass**

   > **Retraction (2026-09-11 20:40 UTC, supersedes gap 10b as filed in `e75962f`/`f3c5993`).** I wrote that the SSE
   > clause "sat inside a list of measurable targets while naming **no threshold at all**, so it could not fail", split
   > it, and substituted a `p95 < 250 ms` target of my own. **That charge was false: the ratified clause reads "in
   > `< 500 ms`"**, which is exactly the falsifiable threshold I claimed was missing — and the grill record's **Q2**
   > quotes it, so the evidence was in a document I had myself written and re-read.
   >
   > **Mechanism: I read the line through `cut -c1-230` and it ended mid-sentence. I then asserted an absence about text
   > I had not looked at.** AGENTS.md already carries this rule — *"filtering a command's output can destroy the only
   > evidence that matters"* — earned by a `grep` that hid a compiler warning, and I broke it reading a spec line
   > instead of a build log. My scripted edit made it worse: `old_string` matched only the clause's first half, so the
   > replacement **severed the sentence** and left `" in < 500 ms;"` dangling, and the severance is what finally
   > revealed the threshold. **A corrupted artifact, not a careful reading, is what caught the corrupted edit.**
   >
   > **What survives of the work, and what does not:** the two-span measurement (wire vs rendered) is retained as
   > **added precision on a target that was always testable**, and the numbers now sit against the owner's `< 500 ms`
   > rather than my invented `250 ms`. What does not survive: gap 10b's premise, its recommendation, and my §7 item —
   > **and specifically the owner's approval of PR #29, which was given on my misstatement** and is recorded below as
   > not-informed. `p95 < 250 ms` is withdrawn; `test/browser/renderLatency.mjs` and its numbers stand.
...
- [x] **(`agent`-owned; was NOT satisfied, now is — grill record §5, 21:10 UTC.)** `grep -ci` over `docs/reviews/2026-09-11-m3-plan-grill.md` before this pass: `ENUM` **0**, `timestamptz` **0**, `fan` **0**, `instance` **0** — so of this item's six required challenges the original pass took *`revision?: number`* (Q5), the *`xmin` counter* (Q6), *`If-Match` on `DELETE`* (Q3), *CORS* (Q4), *§2's targets* (Q2) and *the eighth variant* (Q11), but **never** `text + CHECK` vs `ENUM`, client-minted ids, `date` vs `timestamptz`, or single-instance SSE fan-out. **Now taken as Q13–Q16** (after this pass: `ENUM` 6, `timestamptz` 5, `fan` 3, `instance` 9). **Running the grill I believed I had already done is what found gap 12 — a live 500 that survived the first pass, 14 verified ACs and five audits of this document.** Four §7 boxes remain the owner's; this one was mine.
- [ ] The measurable targets in §2 are falsifiable by a command; if any cannot be, it is deleted rather than defended — **left open deliberately, and it is now much closer than it was.** **Five of six targets are measured and pass**: GET 2.7/5.3 ms, POST p95 5.8 ms, SSE-in-second-tab 33/88 (cold) and 41/58 (warm) against `< 500 ms`, API suite 29 s against `< 60 s` (a **proxy** — CI's own `runDuration` still unread), bundle **+1.21 kB against `< 2 kB`** with a real pre-M3 baseline. **The sixth is the reason this box stays unchecked**: `0 unhandled exceptions` is falsifiable and **passes as written while failing as intended** (gap 12's 500), so the owner must either restate it as "no client-input error may surface as 5xx" — which gap 12 then fails — or strike it. §7 says delete rather than defend; **I am not checking a box on the strength of a target I have just shown to be satisfiable and useless.**
