...
| T-02 | Register/confirm the zone, request **both** ACM certificates, complete DNS validation | T-01 | L1 | `aws acm describe-certificate` → `Status=ISSUED` in **both** us-east-1 and the workload region | DNS validation waits minutes-to-hours and no one wants that inline on deploy day. It is also the only dependency in the milestone that isn't ours to fix. |
| T-03 | Provision VPC (2 AZ: public + private + NAT), ECR repository, CloudWatch log group **with retention** | T-01 | L2 | `aws ec2 describe-subnets` shows 6 subnets across 2 AZs; a private-subnet task reaches the internet; log group has `retentionInDays` set | Networking is what makes "the last hop is cleartext" acceptable (§3.3). Retrofitting a VPC under a live service is the worst version of this task. |
...
**Ordering that is not arbitrary.** T-01 → T-02 → T-03 serialise on external waits (certificates, subnet IDs). **T-06 and
...
## 12. Owner decisions — all seven answered 2026-09-15; the six blockers are cleared (B's zone *name* still open inside its decided shape)
