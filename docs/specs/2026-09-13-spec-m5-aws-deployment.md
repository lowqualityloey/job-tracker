...
## 12. Owner decisions (seven, all open, six of them blocking)
...
reads as finished — that is the failure mode §1.1 documents and §13 counts. **Each row stays `Open` until the owner
writes in the last column**, and T-01's exit criterion is literally "§12 has zero `Open` rows".
