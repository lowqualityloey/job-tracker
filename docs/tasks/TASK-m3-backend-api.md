...
- **TDD Intent Register [Required when enabled]**: `N/A - pending pk:test` — intents will live at [`docs/tests/TEST-m3-backend-api.md`](../tests/TEST-m3-backend-api.md) as `TDD-INTENT-m3-backend-api-<nnn>`; that document does not exist yet and **readiness for `in_progress` requires it to agree with §4's mode**
