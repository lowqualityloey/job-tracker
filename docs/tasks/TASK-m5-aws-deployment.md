...
- **State**: **`handoff_ready`** (2026-09-15 02:06 UTC, [`checkpoint-004`](./TASK-m5-aws-deployment.checkpoint-004.md) — and this field is the
  reason that record exists: **the stop state is written into the Local Task Source itself, not only into `docs/STATE.md` and the checkpoint
  record**, which is precisely the omission checkpoint-003 left behind and the paragraph below names. Resume to `in_progress` only when
  **O-4** arrives and that record's §8 resume condition is validated by the receiver. *Previously:* resumed at 14:35 UTC after
