...
| `…-075` ◆ | `-075` | **One browser-harness recipe instead of two `connect()`s and three boot scripts.** `httpCrossTab.mjs` and `csrfGate.mjs` each carry their own CDP connect / openTab / authenticate block (~150 lines duplicated), and `run-043.sh`, `run-063.sh`, `run-064.sh` each re-implement the scratch-DB + self-signed-cert + `vite build` + in-container-run sequence — including two fixes made twice today (`DROP DATABASE … WITH (FORCE)`, and killing `JobTracker.Api` by exact name because `dotnet run` does not forward signals). Extract, then **re-run both existing rows against the extraction**: a refactor of evidence tooling that is not verified by re-producing the evidence is just editing | Build/infra | p2 |
