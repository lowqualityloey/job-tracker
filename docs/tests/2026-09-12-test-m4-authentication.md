...
| `…-053` | `-053` | Wrong-email and wrong-password are **byte-identical** in body and status, and the **dummy verify's presence is proven structurally**. Timing is **reported, never asserted** (grill Q8) | Integration | p0 |
...
| `…-063` | `-063` | A cross-site write without `X-CSRF-Token` is rejected — **and the same call with the token succeeds.** Without the positive control a broken endpoint passes as a secured one | Browser | p0 |
