...
| `…-074` ◆ | `-074` | **The data-protection key ring outlives the process that made it, on every instance.** `-063`'s token is readable only by the host that minted it — measured, not theorised: `OwnershipTests` built a fresh `WebApplicationFactory` per request and six of its own writes came back `403 antiforgery`. Needs shared key persistence (`PersistKeysTo*` over a store every instance can reach) plus an integration case that a token issued on one host validates on another. Until then the deployed behaviour is **a session may only be written to by the instance that issued it**, which behind a load balancer is a user-visible fault, not a theoretical one · **executed 2026-09-13 (`-074`; its commits are `6300821` Red, `06535c0` Green — a PR number cannot be written before the PR exists)**: the ring now lives in `data_protection_keys`, and the claim that replaced this row's premise is that the fault is visible on a **single instance after one restart** — measured 201 → restart → 403 → fixed to 201 with one key row surviving | Integration | p1 |
...
| `…-076` ◆ | `-076` | **AC-14's bundle gate is not wired into anything.** `-065` established the number (1,242 B under a 3,072 B gate) and the reproduction is byte-exact, but `grep -c bundleDelta .github/workflows/ci.yml` → **0**: nothing re-checks it, so a dependency bump that adds 2 kB reaches `main` with every job green. Wiring it means pinning the URL literal in the workflow (the gate is meaningless without it) and accepting a second build per PR; the alternative is recording the gate as a point-in-time measurement, which is what it is today · **executed 2026-09-13**: the gate is `tests/build/bundleGate.sh` in the `verify` job, the fixture is `tests/build/bundle-baseline.json`, and a dispatch-only `bundle-baseline-recheck` job re-derives `41b32af` and asserts the fixture still matches — the quote is audited, not trusted | Build/infra | p2 |
...
Spec §6 carries the matching amendment for `-074`/`-075`/`-076`.
